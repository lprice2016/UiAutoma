...
def get_device_report_info(report_file):
    ...
    return result
def checkReportFile(FileName):
    reportPath = os.path.join(test_report_path,FileName)
...
def zip_report(mobileName):
    '''压缩TestReport文件夹'''
    global zipFilePath,file_news
    zipFilePath = mobileName + '麦芽Android UI自动化测试' + time.strftime("%Y-%m-%d %H.%M.%S", time.localtime())
    startdir = "./TestReport"  # 要压缩的文件夹路径
    file_news = './' + zipFilePath + '.zip'  # 压缩后文件夹的名字
    ...
    z.close()
    print("-----zipFIlereportPath-------",file_news)
    print('Generate zip_report file %s completed........ ' % file_news)
def getZipFilePath(mobileName):
    """获取压缩文件的路径"""
    rootPath = os.path.split(os.path.realpath(__file__))[0]
    zipFilePath = os.path.join(rootPath,"TestReport",mobileName,"testreport.html")
    return zipFilePath
