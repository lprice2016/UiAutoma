from common.utils.Driver import back, scroll
from FundApp import (MinePage,
                 HomePage,
                 LoginPage,
                 WelcomePage,
                 MockPage)
from FundApp.PageObject.OpenAccountPage import OpenAccountPage
from common.utils.test_utils import *
from common.driver.chrome.chromedriver import ChromeDriver
...
def open_account_steps(d, name='李一'):
    ...
    time.sleep(5)
    driver = ChromeDriver(u2.connect(get_phone_ip())).driver()
    time.sleep(5)
...
def sign_up_steps(d, mobile, password="it789123", code="888888"):
    '''注册步骤'''
    #输入手机号码
    etl = d(resourceId="com.maiyafenqi:id/phone_et")
    etl.set_text("{}".format(mobile))
    time.sleep(0.5)
    #输入图片验证码
    etl_yzm = d(resourceId="com.maiyafenqi:id/code_et")
    etl_yzm.set_text("8888")
    time.sleep(0.5)
    #点击获取短信验证码
    d(resourceId="com.maiyafenqi:id/sms_code_tv").click()
    time.sleep(0.5)
    #输入短信验证码
    etl_dxyzm =d(resourceId="com.maiyafenqi:id/sms_code_et")
    etl_dxyzm.set_text("{}".format(code))
    time.sleep(0.5)
    print("userNameMobile:",mobile)
    #设置密码
    etl_password = d(resourceId="com.maiyafenqi:id/password_et")
    etl_password.set_text("{}".format(password))
    time.sleep(1)
    #收起软键盘
    d.long_click(0.925, 0.517,1)
    time.sleep(1)
    #勾选协议
    d(resourceId="com.maiyafenqi:id/agreement_iv").click()
    time.sleep(0.5)
    #点击"一键注册"按钮
    d(resourceId="com.maiyafenqi:id/commit_tv").click()
    time.sleep(0.5)
...
def register_and_login(d,username=None):
    ...
    d(text="下一步").click()
    ele = Driver.d(resourceId='com.nonoapp:id/et_dialog_img_code')
    if ele.exists(timeout=3):
        ele.click()
        ele.set_text(text='8888')
        d(resourceId='com.nonoapp:id/rl_dialog_common_positive_btn_part').click()
...
def reset_and_start_nonoapp(d, pkg_name='com.maiyafenqi'):
    '''重置并重启App'''
...
def set_env(d):
    '''设置测试环境'''
    env = "STB环境"
    #d(resourceId="com.maiyafenqi:id/item_env_tv", text="{}".format(env.upper())).click_exists(timeout=2)
    d(resourceId="com.maiyafenqi:id/item_env_tv", text="STB环境").click()
    # HomePage().mine_view.click()
...
def start_nonoapp(d, pkg_name='com.maiyafenqi'):
    '''启动App'''
    d.app_stop(pkg_name)
